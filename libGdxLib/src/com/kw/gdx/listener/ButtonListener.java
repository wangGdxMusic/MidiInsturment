...
public class ButtonListener extends MClickListener {
    ...
    public void clicked(InputEvent event, float x, float y) {
        ...
        if(effect && target!=null){
            ...
            target.addAction(Actions.sequence(Actions.delay(0.1667F * releaseTimeScale),
    ...
    protected void release() {
        ...
        if (target != null) {
            ...
            releaseTimeScale = Math.abs((scaleOriginX - target.getScaleX())/(scaleOriginX*0.1F));
            target.addAction(Actions.sequence(Actions.scaleTo(scaleOriginX, scaleOriginY,0.2667F * releaseTimeScale, Interpolation.slowFast), Actions.run(new Runnable() {
                @Override
                public void run() {
                    working = false;
                }

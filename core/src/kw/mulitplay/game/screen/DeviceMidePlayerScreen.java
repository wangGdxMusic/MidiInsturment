...
public class DeviceMidePlayerScreen extends BaseScreen {
...
//        String arg1 = "", arg2 = "";//准备读第二、三个参数
        ...
        try {
            ...
            Synthesizer midid = (Synthesizer) this.midid;
            midid.loadAllInstruments(soundbank);
            Instrument[] availableInstruments = midid.getAvailableInstruments();
            for (Instrument availableInstrument : availableInstruments) {
                System.out.println(availableInstrument.getName());
            }
            midip.getTransmitter().setReceiver(this.midid.getReceiver());//关键的一步，把之前的音序器的Transmitter和MidiDevice的Receiver挂接起来
